...
*mixer, *q1, *jitter1, *jitter2, *textoverlay1, *textoverlay2, *convert1, *convert2;
	...
	videosink = gst_element_factory_make("fpsdisplaysink", "sink");
	...
	//g_object_set(G_OBJECT(source1), "location", "127.0.0.1", "latency", 0, "protocols", 0x00000001, NULL);
	//g_object_set(G_OBJECT(source2), "location", "127.0.0.1", "latency", 0, "protocols", 0x00000001, NULL);
	...
	g_object_set(G_OBJECT(q1), "max-size-buffers", 100, "leaky", 1, "max-size-time", 100000000, NULL);
...
!(gst_element_link_many(source2, jitter2, depay2, decoder2, textoverlay2, convert2, NULL))){
	...
	if(!gst_element_link_many(mixer, q1, videosink, NULL)){
